...
class Insentif extends CI_Controller{
	...
	public function index(){
		if($this->role == 'admin' || $this->role == 'admin-branch'){
			...
			$data['list'] = $this->insentif->get_dataTable($find);
			$this->template->load('layout/admin','master_data/insentif/index', $data);
		}else{
			show_404();
		}
	}
	...
	public function insert(){
		if($this->input->is_ajax_request() && ($this->role == 'admin' || $this->role == 'admin-branch')){
			...
			$p['nominal']   = $p['formula'] != 'none' ? format_angka($p['nominal']) : '';
			$p['is_active'] = isset($p['is_active']) && $p['is_active'] == '1' ? '1' : '0';

			$this->form_validation->set_data($p);
			$this->form_validation->set_rules('insentif_name', 'Nama jabatan', 'required');
			...
			if($this->form_validation->run() == TRUE){
				$find = [
				    'insentif_name' => $p['insentif_name'],
				    'branch_id'     => $p['branch_id']
				];
         

				$cek = $this->insentif->get_detail($find);
				if($cek->num_rows() == 0){
					$p['created_at'] = date('Y-m-d H:i:s');

					if($this->insentif->insert($p)){
						$res = [
							'status'  => true,
	...
	public function update(){
		if($this->input->is_ajax_request() && ($this->role == 'admin' || $this->role == 'admin-branch')){
			$p = $this->input->post();
			$id = $p['id_insentif']; unset($p['id_insentif']);
			...
			$unique = [
				'insentif_name'  => 'Nama Insentif'
			];
			$status = true; $title = '';

			$cek = $this->insentif->get_detail('id', $id)->row_array();
			foreach ($unique as $row => $val) {
				$find[$row]        = $p[$row];
				$find['branch_id'] = $p['branch_id'];
				$exist = $this->insentif->get_detail($row, $p[$row]);
				if($exist->num_rows() > 0){
			...
			if($this->form_validation->run() == TRUE){
				...
				if($status){
					$data = $this->insentif->get_detail('insentif.id', $id);
					if($data->num_rows() > 0){
						$p['updated_at'] = date('Y-m-d H:i:s');

						if($this->insentif->update($p, $id)){
							$res = [
								'status'  => true,
	...
	public function delete(){
		if($this->input->is_ajax_request() && ($this->role == 'admin' || $this->role == 'admin-branch')){
			$find = [
				'insentif.id' => $this->input->post('id'),
				'insentif.branch_id' => $this->userdata->branch_id
			];
			$data = $this->insentif->get_detail($find)->row_array();
			if($this->insentif->delete($this->input->post('id'))){
				$res = [
					'status'  => true,
		...
		if($this->input->is_ajax_request()){
			...
			if($check->num_rows() > 0){
				...
				if($page == 'form'){
					...
					$comp['insentif'] = $this->presence->get_insentif($employee_id, $branch_id, $p['month'], $p['year']);
					$table = '_insentifForm';
