...
require_once __DIR__ . '/absen_report_employee_filter.php';

function generateAbsenReport(mysqli $mysqli, int $year, int $month, ?string $branch = null, ?string $employeeCodesRaw = null, ?string $scopeLabel = null, ?string $asOfRaw = null): array
{
    $periodStart = sprintf('%04d-%02d-01', $year, $month);
    $periodEnd = date('Y-m-t', strtotime($periodStart));
    $asOf = $asOfRaw ?: date('Y-m-d');
    if ($asOf < $periodStart) {
        $asOf = $periodStart;
    }
    if ($asOf > $periodEnd) {
    ...
    $employeeCodes = parseEmployeeCodes($employeeCodesRaw);
    $branchFilter = $employeeCodes ? null : resolveBranchFilter($mysqli, $branch);
    ...
    return [
        ...
        'anomalies' => count($report['anomalies']),
    ];
}
