...
export default function App() {
  ...
  const switchKind = (k) => {
    if (k === kind) return;
    setKind(k); setRows([]); setLoaded(false); setDirty(new Set()); setOpenKey(null); setError(''); setSearch('');
  };
    ...
    <div className="app">
      ...
      {loaded && (
        <div className="toolbar" style={{ marginTop: 0 }}>
          <div className="tb-group" style={{ flex: 1 }}>
            <span className="tb-label">Cari</span>
            <input
              type="search"
              className="tb-select"
              style={{ flex: 1, minWidth: 200 }}
              placeholder="Nama atau ID mitra kerja…"
              value={search}
              onChange={e => setSearch(e.target.value)}
            />
          </div>
          {search && (
            <span className="range-info">{filteredRows.length} dari {rows.length} baris cocok</span>
          )}
        </div>
      )}
      ...
      {loaded && (
        rows.length ? (
          filteredRows.length ? (
            ...
            <div className="empty-note">Tidak ada mitra kerja yang cocok dengan pencarian "{search}".</div>
          )
        ) : (
